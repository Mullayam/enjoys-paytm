...
Connecting to a room requires two parameters: a `url` and a `token`. The `url` points to your LiveKit server instance, which, if you followed  , is likely `ws://localhost:7880`.

The `token` is an access token representing the particular local participant. You can use the aformentioned guide to generate a token via CLI, or you can use our   to create one.
